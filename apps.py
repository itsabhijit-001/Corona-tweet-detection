...
def sentiment_predict():
    ...
    if request.method =='POST':
        ...
        print(tweet)
        token=bagg_tokenizer.transform([tweet]).toarray()
        print(token)
        # pad_seq=pad_sequences(token,maxlen=60,padding='post',truncating='post')
        # print(pad_seq)
        preds = bagged_model.predict(token)
        predictions="The above typed tweet is {0}".format(bagged_lbl_encoder.inverse_transform(preds)[0])
